...
// Optimal Approach:
// So initially, take the first element of the preorder traversal, because it's the root element.
...
// Now break down the problem into two parts, write the inorder and preorder traversals for each sub tree.
// While performing the above steps, you'll get to know about the positions or indexes of the inorder traversal of left sub tree and the preorder traversal of left sub tree => using this we again determine the left sub tree and right sub tree of this particular left sub tree.
// Similarly you'll get to know about the positions or indexes of the inorder traversal of right sub tree and the preorder traversal of right sub tree => using this we again determine the left sub tree and right sub tree of this particular right sub tree.
...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        TreeNode* root = buildingTree(preorder, 0, preorder.size() - 1, inorder, 0, inorder.size() - 1, mpp);

        return root;
    }

    TreeNode* buildingTree(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int, int>& mpp){
        ...
        root -> left = buildingTree(preorder, preStart + 1, preStart + numsLeft, inorder, inStart, inRoot - 1, mpp);
        root -> right = buildingTree(preorder, preStart + numsLeft + 1, preEnd, inorder, inRoot + 1, inEnd, mpp);
        return root;
    }
};