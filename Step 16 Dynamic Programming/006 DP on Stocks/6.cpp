...
class Solution {
    int helper(int ind, int buy, int n, vector<int>& prices, vector<vector<int>>& dp, int fee) {
        if (ind == n) return 0;
        ...
        if (buy == 1) {
            profit = max(-prices[ind] + helper(ind + 1, 0, n, prices, dp, fee),
                         0 + helper(ind + 1, 1, n, prices, dp, fee));
        } else {
            profit = max(prices[ind] - fee + helper(ind + 1, 1, n, prices, dp, fee),
                          0 + helper(ind + 1, 0, n, prices, dp, fee));
        }
